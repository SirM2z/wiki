...
### `exec / test` 的局限性
用 `exec` 或 `test` 捕获字符串，当设置了 `g` 时，这两个方法会修改正则的 `lastIndex` 值，而影响下一次 `exec` 或 `test` 的结果。如下用法，即 `test` 影响了 `exec` 的结果：
```js
const str = 'abc123';
const reg = /\d+/g;
if (reg.test(str)) { // 被 test 方法捕获
  reg.exec(str); // null
}
```
...
### 时间格式化
字符串 `2019/8/26 11:10:20` 转换为 `8-26 11:10`

使用 `spilt` 方法：
```js
const str = '2019/8/26 11:10:20';
const [, M, D, h, m] = str.split(/(?:\/| |:)/g); // ["2019", "8", "26", "11", "10", "20"]
console.log(`${M}-${D} ${h}:${m}`);
```
使用 `match` 方法：
```js
const str = '2019/8/26 11:10:20';
const [, M, D, h, m] = str.match(/\d+/g); // ["2019", "8", "26", "11", "10", "20"]
console.log(`${M}-${D} ${h}:${m}`);
```
